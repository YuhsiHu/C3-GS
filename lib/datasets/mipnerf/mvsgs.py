...
class Dataset:
    ...
    def build_metas(self):
        if len(self.scenes) == 0:
            # scenes = ['bicycle', 'bonsai', 'counter', 'flowers', 'garden', 'kitchen', 'room', 'stump', 'treehill'] # all
            # scenes = ['bicycle', 'flowers', 'garden', 'stump', 'treehill'] # can
            scenes = ['bonsai']
        else:
            scenes = self.scenes
        self.scene_infos = {}
        self.metas = []
        pairs = torch.load('data/mvsgs/pairs_mipnerf.th')
        for scene in scenes:
            ...
            img_paths = sorted([item for item in os.listdir(os.path.join(self.data_root, scene, 'images_4')) if '.JPG' in item])
