...
class Dataset:
    ...
    def build_metas(self):
        if len(self.scenes) == 0:
            # intermediate
            # scenes = ['Train', 'Truck', 'Family', 'Francis', 'Horse', 'Lighthouse', 'M60', 'Panther', 'Playground'] # all
            scenes = ['Train', 'Family', 'Horse', 'M60', 'Playground']
            # advanced
            # scenes = ['Auditorium', 'Ballroom', 'Courtroom', 'Museum', 'Palace', 'Temple'] # all
            # scenes = ['Auditorium', 'Temple']
        else:
            scenes = self.scenes
        self.scene_infos = {}
        self.metas = []
        pairs = torch.load('data/mvsgs/pairs_tnt.th')
        for scene in scenes:
