...
def process_mvsgs(image_stats):
    """
    add image stats to tensorboard for mvsgs
    """
    for k, v in image_stats.items():
        if v.dim() == 4:
            image_stats[k] = torch.clamp(v[0], min=0., max=1.)
        elif v.dim() == 3:
            image_stats[k] = torch.clamp(v, min=0., max=1.)
    return image_stats
...
class Recorder(object):
    ...
    def record(self, prefix, step=-1, loss_stats=None, image_stats=None):
        ...
        # too much disk space for image stats, only record loss stats
