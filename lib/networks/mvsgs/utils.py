...
def depth2point(depth, extrinsic, intrinsic):
    ...
    pts_2d[..., 2] = depth
    # The coordinates are adjusted by subtracting the principal point (cx, cy) from the intrinsic matrix.
    pts_2d[:, :, :, 0] -= intrinsic[:, None, None, 0, 2]
    pts_2d[:, :, :, 1] -= intrinsic[:, None, None, 1, 2]
    # Scale Pixel Coordinates by Depth, image to camera coordinate
    pts_2d_xy = pts_2d[:, :, :, :2] * pts_2d[:, :, :, 2:] # [B, H, W, 2]
    # Concatenate Depth Information Back
    pts_2d = torch.cat([pts_2d_xy, pts_2d[..., 2:]], dim=-1) # [B, H, W, 2] [B, H, W, 1] -> [B, H, W, 3]
    # The x and y coordinates are normalized by the respective focal lengths (fx, fy) from the intrinsic matrix.
...
def get_proj_mats(batch, src_scale, tar_scale):
    ...
    proj_mats = src_projs @ tar_projs_inv # [B, S, 3, 4]
    return proj_mats
...
def get_depth_values(batch, D, level, device, depth, std, near_far):
    ...
    if depth is None:
        # calculates depth values using the near-far plane
        if cfg.mvsgs.cas_config.depth_inv[level]:
            # disp_values = 1/near + (linspace * (1/far - 1/near))
            ...
            # depth_values = near + (linspace * (far - near))
        ...
        depth_values = depth_values.view(B, D, 1, 1).repeat(1, 1, H, W) # [B, D, H, W]
    else:
        up_scale = cfg.mvsgs.cas_config.volume_scale[level] / cfg.mvsgs.cas_config.volume_scale[level-1]
        if up_scale != 1.:
            depth = F.interpolate(depth[:, None], None, scale_factor=up_scale, recompute_scale_factor=True, align_corners=True, mode='bilinear')[:, 0] # [B, H, W]
            std = F.interpolate(std[:, None], None, scale_factor=up_scale, recompute_scale_factor=True, align_corners=True, mode='bilinear')[:, 0] # [B, H, W]
            near_far = F.interpolate(near_far, None, scale_factor=up_scale, recompute_scale_factor=True, align_corners=True, mode='bilinear') # [B, 2, H, W]
        ...
        if cfg.mvsgs.cas_config.depth_inv[level]:
            ...
            depth_values = near_far[..., :1] + torch.linspace(0., 1., steps=D, device=device, dtype=torch.float32).view(1, 1, 1, -1).repeat(B, 1, 1, 1) * (near_far[..., 1:] - near_far[..., :1]) # [B, H, W, D]
            depth_values = depth_values.permute((0, 3, 1, 2)) # [B, D, H, W]
    near_far = depth_values[:, [0, -1], :, :].detach() # [B, 2, H, W]
    if cfg.mvsgs.cas_config.depth_inv[level]:
...
def build_feature_volume(feature, batch, D, depth, std, near_far, level):
    ...
    Args:
        ...
        level: int
    Returns:
        feature_volume: [B, C+C, D, H, W]
        depth_values: [B, D, H, W]
        near_far: [B, 2, H, W]    
    """
    B, S, C, H, W = feature.shape
    depth_values, near_far = get_depth_values(batch, D, level, feature.device, depth, std, near_far) # [B, D, H, W] [B, 2, H, W]

    proj_mats = get_proj_mats(batch, src_scale=cfg.mvsgs.cas_config.im_feat_scale[level], tar_scale=cfg.mvsgs.cas_config.volume_scale[level]) # [B, S, 3, 4]

    volume_sum = 0
    volume_sq_sum = 0
    count = 0
    # source view aggregation
    for s in range(S):
        feature_s = feature[:, s]
        proj_mat = proj_mats[:, s]
        warped_volume, _ = homo_warp(feature_s, proj_mat, depth_values) # [B, C, D, H, W]
        volume_sum = volume_sum + warped_volume
        volume_sq_sum = volume_sq_sum + warped_volume ** 2
    volume_variance = volume_sq_sum.div_(S).sub_(volume_sum.div_(S).pow_(2)) # [B, C, D, H, W]
    volume_context = volume_sum.div_(S) # [B, C, D, H, W]
    # why concat mean and var? based on experiments
    feature_volume = torch.cat((volume_variance, volume_context),dim=1) # [B, C+C, D, H, W]
...
def build_rays(depth, std, batch, training, near_far, level, up_scale=2.):
    ...
    rays = batch[f'rays_{level}'] # the same as rays_o, rays_d, pixel_coordinates from get_rays_mvs in mvsnerf [B, H*W, 8]
    if cfg.mvsgs.cas_config.depth_inv[level]:
        ...
        mask = rays_near_far[..., 1] > near_far[:, 1]
        rays_near_far[..., 1][mask] = near_far[:, 1][mask]
    near_far = near_far.permute(0, 2, 3, 1) # [B, H, W, 2]
    rays = batch[f'rays_{level}']
    uv = rays[:, :, 6:].long() # [B, H*W, 2]
    rays_near_far = torch.stack([rays_near_far[i][uv[i][:, 1], uv[i][:, 0]] for i in range(len(rays_near_far))]) # [B, H*W, 2]
    near_far = torch.stack([near_far[i][uv[i][:, 1], uv[i][:, 0]] for i in range(len(near_far))]) # [B, H*W, 2]
    rays = torch.cat([rays, rays_near_far, near_far], dim=-1) # # [B, H*W, 12]
...
def sample_along_depth(rays, N_samples, level):
    ...
    Args:
        rays: [B, H*W, 12]
        N_samples: 1, hyperparam that can be set in yaml
        level: 0 or 1
    Returns:
        world_xyz: [B, H*W, 1, 3]
        uvd: [B, H*W, 1, 3]
        z_vals: [B, H*W, 1]
    """
    ray_o, ray_d, uv = rays[..., :3], rays[..., 3:6], rays[..., 6:8] # [B, H*W, 3], [B, H*W, 3], [B, H*W, 2]
    ray_near, ray_far, near, far = rays[..., 8:9], rays[..., 9:10], rays[..., 10:11], rays[..., 11:12] # [B, H*W, 1], [B, H*W, 1], [B, H*W, 1], [B, H*W, 1]
    if N_samples == 1:
        z_vals =  ray_near + (ray_far - ray_near) * 0.5 # [B, H*W, 1]
    else:
    ...
    return world_xyz, uvd, z_vals, ray_near, ray_far
...
def get_vox_feat(ndc_xyz, feature_volume):
    ...
    Returns: 
        feature: [B, H*W, C]
    """
    # ndc_xyz: [B, H*W, 3] -> [B, 1, 1, H*W, 3], then grid_sample
    feature = F.grid_sample(feature_volume, ndc_xyz[:, None, None]*2. - 1., align_corners=True)[:, :, 0, 0].permute((0, 2, 1)) # [B, C, 1, 1, H*W] -> [B, C, H*W] -> [B, H*W, C]
    return feature
...
def get_img_feat(xyz, img_feat_rgb, batch, training, level):
    """Extracts image features and ray direction differences for each 3D sample point across multiple source views.
    Called at: network.py, img_feat_rgb_dir = get_img_feat(xyd, img_feat_rgb, batch) # [B, H*W, S, 3+C+4], C=32 or 8
    Args:
        xyz: [B, H*W, 1, 3]
        img_feat_rgb: [B, S, 3+C, H, W]
        batch: info from dataloader
        training: bool
        level: 0 or 1
    Returns:
        ret_feat: [B, H*W, S, 3+C+4]
    """
    B, S, C, H, W = img_feat_rgb.shape
    B, N_rays, N_samples = xyz.shape[:3] # N_rays = H*W, N_samples = 1
    xyz = xyz.view(B, N_rays * N_samples, xyz.shape[-1]) # [B, H*W, 3]
    xyz = torch.cat([xyz, torch.ones_like(xyz[..., :1])], dim=-1) # [B, H*W, 4]
    ...
    for i in range(S):
        ...
        grid = grid * 2. - 1.
        feat = F.grid_sample(img_feat_rgb[:, i], grid[:, None], align_corners=True, mode='bilinear', padding_mode='border').permute(0, 2, 3, 1)[:, 0] # [B, H*W, 3+C]
        ...
        ray_diff = torch.cat([ray_diff_direction, ray_diff_dot], dim=-1) # [B, H*W, 4]
