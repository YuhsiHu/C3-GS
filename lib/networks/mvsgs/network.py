...
class Network(nn.Module):
    def __init__(self,):
        ...
        for i in range(cfg.mvsgs.cas_config.num):
            if i == 0:
                # shallow regnet because D is large
    ...
    def render_rays(self, rays, **kwargs):
        level, batch, im_feat, feat_volume, gs_model, size, prev_gaussians = kwargs['level'], kwargs['batch'], kwargs['im_feat'], kwargs['feature_volume'], kwargs['gs_model'], kwargs['size'], kwargs['prev_gaussians']
        # world_xyz, uvd, z_vals = utils.sample_along_depth(rays, N_samples=cfg.mvsgs.cas_config.num_samples[level], level=level) # [B, H*W, 1, 3] [B, H*W, 1, 3] [B, H*W, 1]
        world_xyz, uvd, z_vals, ray_near, ray_far = utils.sample_along_depth(rays, N_samples=cfg.mvsgs.cas_config.num_samples[level], level=level) # [B, H*W, 1, 3] [B, H*W, 1, 3] [B, H*W, 1]
        B, N_rays, N_samples = world_xyz.shape[:3]
        # get original source images
        rgbs = utils.unpreprocess(batch['src_inps'], render_scale=cfg.mvsgs.cas_config.render_scale[level]) # [B, S, 3, H, W]
        # feature scaling
        ...
        # concat source features and source images
        img_feat_rgb = torch.cat((im_feat, rgbs), dim=2) # [B, S, C, H, W] [B, S, 3, H, W] -> [B, S, 3+C, H, W]
        # normalize uv to [0, 1]
        ...
        vox_feat = utils.get_vox_feat(uvd.reshape(B, -1, 3), feat_volume) # [B, H*W, 1, 3] [B, C, D, H, W] -> [B, H*W, C]
        # image feature extraction
        img_feat_rgb_dir = utils.get_img_feat(world_xyz, img_feat_rgb, batch, self.training, level) # [B, S, 3+C, H, W] -> [B, H*W, S, 3+C+4], C=32 or 8
        # rendering
        net_output = gs_model(vox_feat, img_feat_rgb_dir, z_vals, batch, size, level, prev_gaussians)
        return net_output
    ...
    def forward(self, batch):
        ...
        if not cfg.save_video:
            feats = self.forward_feat(batch['src_inps']) # ['level_0'] ['level_1'] ['level_2'] [B, S, C, H, W]
            ret = {}
            ...
            for i in range(cfg.mvsgs.cas_config.num):
                ...
                feature_volume, depth_values, near_far = utils.build_feature_volume(
                        ...
                        std=std,
                        near_far=near_far,
                        level=i) # feature_volume: [B, C+C=64, D, H, W] depth_values: [B, D, H, W] near_far: [B, 2, H, W]
                feature_volume, depth_prob = getattr(self, f'cost_reg_{i}')(feature_volume) # cost_volume: [B, G, D, H, W] depth_prob: [B, 1, D, H, W]
                depth, std = utils.depth_regression(depth_prob, depth_values, i, batch) # depth: [B, H, W] std: [B, H, W]
                # get output based on depth and cost volume
                rays = utils.build_rays(depth, std, batch, self.training, near_far, i) # UV(2) + ray_o (3) + ray_d (3) + ray_near_far (2) + volume_near_far (2) [B, H*W, 12]
                ...
                #### occloss ####
                world_xyz, rot_out, scale_out, opacity_out, color_out, rgb_vr, sigma, x = output
                #### erroraware ####
                x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2) # [B, C, H, W]
                x = F.interpolate(x, size=(H_img, W_img), mode='bilinear', align_corners=True).permute(0, 2, 3, 1).reshape(B, H_img*W_img, -1) # [B, H_img*W_img, C]
                ####################
                # when test, this is [False, True], so there is no render in low resolution
                if not cfg.mvsgs.cas_config.render_if[i]:
                    continue
                render_novel = []
                # render_depth = []
                for b_i in range(B):
                    render_novel_i_0, render_depth_i_0 = render(batch[f'novel_view{i}'], b_i, world_xyz[b_i], color_out[b_i], rot_out[b_i], scale_out[b_i], opacity_out[b_i], bg_color=cfg.mvsgs.bg_color)
                    ...
                    # render_depth.append(render_depth_i_0)
                render_novel = torch.stack(render_novel) # [B, 3, H, W]
                ret_i.update({'rgb': render_novel.flatten(2).permute(0,2,1)})
                # #### monodepthloss ####
                # render_depth = torch.stack(render_depth) # [B, 1, H, W]
                # ret_i.update({'render_depth': render_depth})
                # #######################
                #### occloss ####
                ret_i.update({'sigma': sigma})
                #################
                if cfg.mvsgs.cas_config.depth_inv[i]:
                    ret_i.update({'depth_mvs': 1./depth})
                    # ret_i.update({'depth_mvs': 1./depth_from_mvs})
                else:
                    ret_i.update({'depth_mvs': depth})
                    # ret_i.update({'depth_mvs': depth_from_mvs})
                ret_i.update({'std': std})
                if ret_i['rgb'].isnan().any():
                    __import__('ipdb').set_trace()
                ret.update({key+f'_level{i}': ret_i[key] for key in ret_i})
                # for visualization
                ret.update({'render_novel_i_0': render_novel_i_0})
                ret.update({'rgb_vr': rgb_vr})
            ...
            for v_i, meta in enumerate(batch['rendering_video_meta']):
                ...
                for i in range(cfg.mvsgs.cas_config.num):
                    ...
                    if i == cfg.mvsgs.cas_config.num-1:
                        world_xyz, rot_out, scale_out, opacity_out, color_out, rgb_vr, sigma, x = output
                        for b_i in range(B):
                            render_novel_i_0, render_depth_i_0 = render(meta, b_i, world_xyz[b_i], color_out[b_i], rot_out[b_i], scale_out[b_i], opacity_out[b_i], bg_color=cfg.mvsgs.bg_color)
                            if cfg.mvsgs.reweighting: 
